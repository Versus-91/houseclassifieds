...
new Vue({
	...
	methods: {
		...
		submit: function (number) {
			...
			form.append('logo', !!this.logo ? this.logo:null);
			form.append('address', this.address);
			form.append('districtId', this.district);
			form.append('file', this.file);
			form.append('PhoneNumbers', this.numbers.join(','));
			axios.post('/admin/realestates/edit', form).then(function (res) {
	...
	mounted: function () {
		...
		axios.get('/api/services/app/realestate/get?id='+id).then( (res) =>{
			...
			this.logo = item.logo;
			this.location = item.district.area.name + " " + item.district.name;
			this.srcDistrict = item.district.id;
			this.district = item.district.id;
			this.numbers = item.phoneNumbers.split(",");
		}).catch(function (err) { });
	}
});
